...
dens <-
  ...
  geom_density(
    ...
    aes(
      ...
      text = sprintf(
        "Densidade: %.2f%%<br>Tempo de Operação: %.2f dias",
...
subplot(box_ly, dens_ly,nrows = 1, margin = 0.05)
```
...
Nota-se que os outliers temporais observados nesta seção são distinos dos citados na @sec-distri, já que àqueles são outliers **globais**, pois destoam da média dos DAPs, enquanto os outliers temporais são **contextuais**, no sentido que são valores anômalos apenas no contexto da tendência das séries temporais de medidas.
