...
full_join(liers, gaps, by = c("Fazenda" = "Fazenda", "Amostra" = "Amostra")) |> 
  mutate(across(c(Amostra, Fazenda), as.factor)) |> 
...
bar_ly <- ggplotly(barplot, tooltip = "text")
...
dens <-
  ...
  geom_density(
    ...
    aes(
      ...
      text = sprintf("Densidade: %.2f%%<br>Tempor de Operação: %.2f dias",
...
subplot(box_ly, dens_ly,nrows = 1, margin = 0.07)
```
